...
pub fn spawn_gridline(mut commands: Commands, window: Res<WindowSize>) {
    ...
    let horizontal_steps = (((-window.height / 2.0 + 100.0) as i32)..=((window.height / 2.0 - 100.0) as i32)).step_by(GRIDLINE_SPACING);
    let vertical_steps = (((-window.width / 2.0 + 100.0) as i32)..=((window.width / 2.0 - 100.0) as i32)).step_by(GRIDLINE_SPACING);

    for line in horizontal_steps {
        commands.spawn((
            Sprite::from_color(gridline.color, Vec2::new(gridline.thickness, gridline.width.unwrap())),
            Transform {
    ...
    for line in vertical_steps {
        commands.spawn((
            Sprite::from_color(gridline.color, Vec2::new(gridline.width.unwrap(), gridline.thickness)),
            Transform {
