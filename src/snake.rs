use bevy::{color::Color, math::bounding::Aabb2d, math::bounding::BoundingVolume, prelude::*};
...
impl Plugin for SnakePlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, (spawn_snake_system, spawn_snake))
            .add_systems(Update, (move_snake, spawn_snake_section))
...
impl Default for SnakePart {
    fn default() -> Self {
        SnakePart {
            ...
            scale: SNAKE_SCALE,
            section_index: 0,
            bounds: Aabb2d::new(Vec2::new(0.0, 0.0), Vec2::new(SNAKE_SCALE, SNAKE_SCALE)),
        }
    }
}
...
pub fn spawn_snake_system(mut commands: Commands) {
    let system_id = commands.register_system(spawn_snake);
    commands.spawn((Callback(system_id), SnakeRequest));
}
...
pub fn spawn_snake(mut commands: Commands) {
    ...
    commands
        .spawn((
            Sprite::from_color(
                SNAKE_HEAD_COLOR,
                Vec2::new(snake_part.scale, snake_part.scale),
            ),
            Direction::Pause,
            snake_part,
        ))
...
pub fn spawn_snake_section(
    ...
    if sections.count < sections.requested {
        ...
        for (transform, snake_part) in &position {
            ...
            if (penultimate_section_index < snake_part.section_index)
                && !(snake_part.section_index <= 0)
            {
        ...
        commands.spawn((
            Sprite::from_color(
                SNAKE_BODY_COLOR,
                Vec2::new(new_snake_part.scale, new_snake_part.scale),
            ),
            Transform {
                translation: last_location.translation
                    - last_location
                        .rotation
                        .mul_vec3(Vec3::new(0.0, new_snake_part.scale, 0.0)),
